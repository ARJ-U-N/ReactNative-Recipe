...
const RecipeDetailScreen = () => {
  const { id: recipyId } = useLocalSearchParams();
  ...
  try {
    const url = `${API_URL}/favorites/${userId}`;
    console.log("✅ Checking saved recipe at:", url);
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      console.log("❌ Received non-JSON response:", errorText);
      throw new Error("Invalid response type");
    }

    const favorites = await response.json();
    const isRecipeSaved = favorites.some(
      (fav) => fav.recipyId === parseInt(recipyId)
    );
    ...
    const loadRecipeDetail = async () => {
      setLoading(true);
      try {
        const mealData = await MealAPI.getMealById(recipyId);
        if (mealData) {
  ...
  const handleToggleSave = async () => {
    ...
    try {
      if (isSaved) {
        ...
        const response = await fetch(`${API_URL}/favorites/${userId}/${recipyId}`, {
          method: "DELETE",
        });
